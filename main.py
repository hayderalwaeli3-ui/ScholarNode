...
# --- التنسيق البصري (CSS) كما هو تماماً ---
...
# --- القائمة الجانبية كما هي تماماً ---
with st.sidebar:
...
with tabs[1]:
    ...
    if st.button(f"بدء ترجمة الملف ({st.session_state.total_pages} صفحة)"):
        if deduct_attempt(st.session_state.total_pages):
            bar = st.progress(0) # إضافة شريط النسبة
            with st.spinner("جاري الترجمة..."):
                for i in range(1, 101): # رفع المؤشر تدريجياً
                    time.sleep(0.01)
                    bar.progress(i)
                uploaded_file.seek(0)
                ...
                st.download_button("📥 تحميل ملف Word", data=create_word_file(res.choices[0].message.content), file_name="translated.docx")
...
with tabs[2]:
    ...
    if st.button(f"بدء المراجعة ({st.session_state.total_pages} صفحة)"):
        if deduct_attempt(st.session_state.total_pages):
            bar_rev = st.progress(0) # إضافة شريط النسبة
            with st.spinner("جاري المراجعة..."):
                ...
                st.write(res.choices[0].message.content)
                st.download_button("📥 تحميل ملف Word", data=create_word_file(res.choices[0].message.content), file_name="reviewed.docx")
        else: st.error("رصيدك غير كافٍ.")
