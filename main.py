...
if uploaded_file:
    # قراءة أولية لمعرفة عدد الصفحات
    ...
    with tabs[1]:
        ...
        if st.button(f"بدء ترجمة الملف ({st.session_state.total_pages} صفحة)"):
            if deduct_attempt(st.session_state.total_pages):
                # إعادة ضبط المؤشر قبل القراءة للترجمة
                uploaded_file.seek(0)
                doc = fitz.open(stream=uploaded_file.read(), filetype="pdf")
                all_text = "\n".join([page.get_text() for page in doc])
                
                # ترجمة النص بالكامل (تقسيمه إذا كان طويلاً جداً)
                res = client.chat.completions.create(model="gpt-4o", messages=[{"role": "system", "content": "You are a professional academic translator."}, {"role": "user", "content": f"Translate the following academic text to {target_lang} completely:\n\n{all_text}"}])
    ...
    with tabs[2]:
        ...
        if st.button(f"بدء المراجعة ({st.session_state.total_pages} صفحة)"):
            if deduct_attempt(st.session_state.total_pages):
                # إعادة ضبط المؤشر قبل القراءة للمراجعة
                uploaded_file.seek(0)
                doc = fitz.open(stream=uploaded_file.read(), filetype="pdf")
                all_text = "\n".join([page.get_text() for page in doc])
                
                res = client.chat.completions.create(model="gpt-4o", messages=[{"role": "system", "content": "You are an expert academic reviewer."}, {"role": "user", "content": f"Review and improve this academic text for clarity, grammar, and formal tone:\n\n{all_text}"}])
