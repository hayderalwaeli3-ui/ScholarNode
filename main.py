...
if up:
    ...
    with tabs[1]: # تبويب الترجمة المطور بمؤشر تقدم
        ...
        if st.button(f"بدء معالجة {st.session_state.total_pages} صفحة"):
            if deduct_attempt(st.session_state.total_pages):
                progress_bar = st.progress(0)
                status_text = st.empty()
                with st.spinner("جاري استخراج وترجمة النص..."):
                    # محاكاة لرفع العداد تدريجياً
                    for i in range(1, 51):
                        time.sleep(0.01)
                        progress_bar.progress(i)
                        status_text.text(f"جاري المعالجة: {i}%")
                    ...
                    st.download_button("📥 تحميل ملف Word", data=create_word_file(final_txt), file_name="translated.docx")
            else: st.error("رصيدك غير كافٍ")

    with tabs[2]: # تبويب المراجعة المطور بمؤشر تقدم
        st.subheader("🎓 المراجعة العلمية")
        if st.button("بدء المراجعة الشاملة"):
            if deduct_attempt(st.session_state.total_pages):
                p_bar = st.progress(0)
                s_text = st.empty()
                with st.spinner("جاري مراجعة الأوراق العلمية..."):
                    ...
                    st.download_button("📥 تحميل ملف Word", data=create_word_file(final_txt), file_name="reviewed.docx")
            else: st.error("رصيدك غير كافٍ")
