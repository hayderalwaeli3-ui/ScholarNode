...
st.markdown("""
<style>
/* الحماية المطلقة وإخفاء القوائم البرمجية */
#MainMenu {visibility: hidden;} footer {visibility: hidden;} header {visibility: hidden;}
.stDeployButton {display:none;}
:root { --text-color: inherit; }

.main-header { 
    background: #1e3a8a; color: #ffffff !important; padding: 20px; 
    text-align: center; border-radius: 15px; border: 4px solid #facc15; margin-bottom: 25px; 
}

.payment-box { 
    background: #1e3a8a; color: white !important; padding: 18px; 
    border-radius: 12px; border: 3px solid #facc15; font-size: 0.95rem; line-height: 1.6; 
}
...
.cost-alert { 
    background: rgba(255, 251, 230, 0.1); border-right: 5px solid #facc15; 
    padding: 15px; color: inherit; font-weight: bold; border-radius: 5px; margin: 10px 0; 
}
</style>
""", unsafe_allow_html=True)

# --- القائمة الجانبية (Sidebar) كاملة المعلومات ---
with st.sidebar:
    ...
    <div class="payment-box">
        ...
        📞 <b>الدعم والتفعيل:</b> 07715632230
    </div>
    """, unsafe_allow_html=True)
    ...
    if adm == "HAYDER_2026":
        cat = st.selectbox("اختر الفئة للتوليد:", [10, 20, 30, 40, 50, 100])
...
# 1. تبويب المستشار الذكي (يعمل بدون ملف وبسياسة مالية موحدة)
with tabs[0]:
    ...
    if c_prompt:
        is_research = any(word in c_prompt for word in ["بحث", "دراسة", "أطروحة", "فصل"])
        cost_att = 10 if is_research else 1
        att_c, iqd_v = calculate_costs(cost_att)
        
        st.warning(f"⚠️ تنبيه مالي: هذه العملية ستكلفك {cost_att} محاولة (ما يعادل {iqd_v} دينار عراقي).")
        if st.button("تأكيد العملية والخصم"):
            if deduct_attempt(cost_att):
                ...
                res = client.chat.completions.create(model="gpt-4o", messages=[{"role": "system", "content": "أنت خبير أكاديمي يكتب بحوثاً رصينة وجاهزة للنشر العلمي."}] + st.session_state.chat_history)
                st.markdown(res.choices[0].message.content)
                st.session_state.chat_history.append({"role": "assistant", "content": res.choices[0].message.content})
                st.download_button("📥 تحميل المخرج (Word)", data=create_word_file(res.choices[0].message.content), file_name="research_output.docx")
...
if up:
    ...
    with tabs[1]:
        ...
        st.markdown(f'<div class="cost-alert">💰 تكلفة الترجمة ({p_count} صفحة): {att_req} محاولة | {iqd_req} دينار عراقي</div>', unsafe_allow_html=True)
        if st.button("تأكيد الترجمة الشاملة والخصم"):
            if deduct_attempt(att_req):
                with st.spinner("جاري الترجمة الاحترافية لجميع الصفحات..."):
                    all_text = "\n".join([p.get_text() for p in doc_v])
                    res = client.chat.completions.create(model="gpt-4o", messages=[{"role": "user", "content": f"Translate this entire academic document to Arabic perfectly:\n{all_text}"}])
                    st.success("✅ اكتملت الترجمة!")
                    st.download_button("📥 تحميل الملف المترجم", data=create_word_file(res.choices[0].message.content), file_name="full_translation.docx")
            else: st.error("الرصيد غير كافٍ")

    with tabs[2]:
        st.subheader("🎓 مراجعة نقدية (جاهزة للنشر)")
        st.markdown(f'<div class="cost-alert">💰 تكلفة المراجعة العلمية: {att_req} محاولة | {iqd_req} دينار عراقي</div>', unsafe_allow_html=True)
        if st.button("توليد التقرير النقدي النهائي"):
            if deduct_attempt(att_req):
                with st.spinner("جاري التحليل العلمي الرصين..."):
                    all_text = "\n".join([p.get_text() for p in doc_v])
                    res = client.chat.completions.create(model="gpt-4o", messages=[{"role": "user", "content": f"Provide a complete, publisher-ready academic critical review for this text: {all_text}"}])
                    st.success("✅ التقرير جاهز!")
                    st.download_button("📥 تحميل التقرير (Word)", data=create_word_file(res.choices[0].message.content), file_name="academic_review.docx")

    with tabs[3]:
        st.subheader("📄 معاينة ومناقشة الملف")
        # تعديل الدكتور: الواجهة في الأعلى
        col_ctrl, _ = st.columns([3, 1])
        with col_ctrl:
            p_idx = st.number_input("الصفحة:", 1, p_count, 1)
            q_prompt = st.text_input("اسأل عن محتوى هذه الصفحة:")
            if st.button("إرسال السؤال للأكاديمي"):
                if deduct_attempt(1):
    ...
    with tabs[1]: st.info("ارفع ملف PDF لتفعيل الترجمة.")
    with tabs[2]: st.info("ارفع ملف PDF لتفعيل المراجعة.")
    with tabs[3]: st.info("ارفع ملف PDF للمعاينة والمناقشة.")

st.markdown("<br><hr><p style='text-align:center;'>ScholarNode Academy © 2026 | النسخة المعتمدة (1)</p>", unsafe_allow_html=True)
